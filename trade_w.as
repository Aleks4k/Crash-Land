...
import fl.transitions.TweenEvent;
 ...
 import com.freeactionscript.Scrollbar;
    ...
    public dynamic class trade_w extends flash.display.MovieClip
    {
		...
		public function cls()
        {
             MovieClip(root).offer_btn.visible = true
		...
		public function setup(param1:String)
        {
			...
			trace("nesta pokusaj jebem ti mrtve");
            return;
        }// end function
		
		
		 public function unlockedOffer()
        {
			this.lockov.visible = false;
			this.ok_btn.visible = true;
			this.waiting.visible = false;
		  ...
		  public function ok_pressed(event:MouseEvent)
        {
            ...
            MovieClip(root).lobbyconnection.send("trade_lock");
            this.lockOffer();
            return;
        }// end function
        ...
        {
            MovieClip(root).lobbyconnection.send("leave_offer");
			 MovieClip(root).offer_btn.visible = true
            MovieClip(parent).removeChild(this);
            return;
        }// end function
	...
	public function item_over(event:flash.events.MouseEvent):*
		   ...
		   public function item_down(event:flash.events.MouseEvent):*
        {
		  this.c_item_id = event.currentTarget.num;
			...
			MovieClip(root).refreshIt();
			trace("a u tri lepe ckepi terinema");
            MovieClip(root).addChild(this.cit);
			this.cit.x = 419.95;
			this.cit.y = 306;
            this.cit.setup(MovieClip(root).myItems, this.itemSelect, []);
            this.cit_open = true;
            return;
		}
           ...
           public function itemSelect(param1:String, param2:String = "")
        {
            ...
            if (param1 == "select")
            {
                ...
                MovieClip(root).lobbyconnection.send("trade_item_update", this.c_item_id, param2);
				trace("noob")
            }
            return;
        }// end function

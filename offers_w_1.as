...
import fl.transitions.TweenEvent;
 ...
 import com.freeactionscript.Scrollbar;
    ...
    public dynamic class offers_w_1 extends flash.display.MovieClip
    {
        ...
        {
			this.getlist = ["", "", "", "", "", "", 0];
            //this.trdr_sound.play();
            this.trade_actions.gotoAndStop(2);
			for(var i=1;i<7;i++){
				this.trade_actions["item" + i].inner.visible = false
		 ...
		 public function lockOffer()
        {
            ...
            this.trade_actions.unlock_btn.visible = true;
		  ...
		  public function ok_pressed(event:MouseEvent)
        {
            ...
            MovieClip(root).lobbyconnection.send("trade_lock", this.partner_mpid);
            this.lockOffer();
            return;
        }// end function
         ...
         public function itemUpdate(param1:int, param2:String)
        {
            ...
            if (currentFrame == 2)
            {
                ...
                {
                    this.trade_actions["item" + param1].inner.gotoAndStop(param2);
                }
                this.getlist[(param1 - 1)] = param2;
            }
            updateAnim(this.trade_actions["item" + param1].inner);
            return;
        }// end function
		...
		public function updateAnim(param1:MovieClip)
        {
            ...
            param1.scaleX = 1.5;
            param1.scaleY = 1.5;
            Tweener.addTween(param1, {scaleX:0.5, scaleY:0.5, time:0.6});
            return;
        }// end function
	...
	public function item_over(event:flash.events.MouseEvent):*
        {
             ...
             caurina.transitions.Tweener.addTween(event.currentTarget, {scaleX:0.5, scaleY:0.5, time:0.5});
        ...
        public function item_out(event:flash.events.MouseEvent):*
        {
            ...
            caurina.transitions.Tweener.addTween(event.currentTarget, {"scaleX":0.5, "scaleY":0.5, "time":0.4});
            return;
        }
		 public function back_item(event:flash.events.MouseEvent):*
        {
            caurina.transitions.Tweener.addTween(event.currentTarget, {"scaleX":0.5, "scaleY":0.5, "rotation":0, "time":0.12});
            return;
        }
		   public function item_down(event:flash.events.MouseEvent):*
        {
		  this.c_item_id = event.currentTarget.num;
            this.cit = new items_table();
        ...
        {
           MovieClip(root).lobbyconnection.send("rejectpartner");
			partnerDisconnected();
			trace("gooo")
            return;
        }// end function
          ...
          public function acceptCurrentPartner()
        {
                ...
                MovieClip(root).lobby_mc.visible = false;
               // this.awaiting_accept_return = true;
                this.accepted_getlist = this.getData();
			trace(this.accepted_getlist);
                MovieClip(root).lobbyconnection.send("trade_accept", this.accepted_getlist,   MovieClip(root).itemoffer );
               
                this.gotoAndStop(1);
              
              
                //this.stopPrivateChat();
                MovieClip(root).tradeDone();
            return;
        }// end function
